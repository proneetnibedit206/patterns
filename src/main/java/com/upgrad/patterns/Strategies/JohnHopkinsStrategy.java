...
public class JohnHopkinsStrategy implements IndianDiseaseStat {
	...
	public String GetActiveCount() {
		...
		try{
			...
			int sum = 0;
			JohnHopkinResponse[] johnHopkinResponses = this.getJohnHopkinResponses();
			for(JohnHopkinResponse johnHopkinResponse : johnHopkinResponses){
				if(johnHopkinResponse.getCountry().equals("India")){
					logger.info("johnHopkinResponses " + johnHopkinResponse.getStats().getConfirmed());
					sum+=johnHopkinResponse.getStats().getConfirmed();
				}
			}
			logger.info("sum " + sum);
